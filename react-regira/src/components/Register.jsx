...
export const Register = () => {
  ...
  const onSubmitForm = () => {
    ...
    const data = JSON.stringify({ email, nombre, password })
    const options = {
      method: "POST",
      headers: {
    ...
    <div className="w-screen">
      <div className="bg-register-img flex justify-center items-center h-screen">
        <div className="w-96 py-10 border-2 rounded-md flex flex-col px-10 bg-white shadow-2xl">
          ...
          <h1 className="pt-7 text-center text-xl">Registrate para tener tu cuenta</h1>
          <div className="py-0">
            <form className="" action="" onSubmit={onSubmitForm}>
              <div className="py-4">
                <input type="text" placeholder="Introduce tu nombre" className="w-full ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#0052CC] border py-2 px-1 rounded-lg" name="nombre" value={nombre} onChange={() => setNombre(event.target.value)} />
                {nombre}
              </div>
              <div className="py-4">
